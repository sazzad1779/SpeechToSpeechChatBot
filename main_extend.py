...
def process_tts_stream(llm_queue, session_id):
    tts_buffer = ""
    
    while True:
        token = llm_queue.get()  # Get next token from LLM
        
        if token is None:  # If LLM is done, exit loop
            break

        tts_buffer += token  # Accumulate words

        if len(tts_buffer) > 50:  # Adjust buffer size (e.g., every 10 characters)
            with model_locks['tts']:
                speech_audio = tts_loader.predict(tts_buffer)  # Convert text to speech
            
            audio_data = base64.b64encode(speech_audio).decode('utf-8')
            socketio.emit('audio', {'audio': audio_data, 'session_id': session_id})

            tts_buffer = ""  # Reset buffer after sending audio

    # Final flush
    if tts_buffer:
        with model_locks['tts']:
            speech_audio = tts_loader.predict(tts_buffer)
        audio_data = base64.b64encode(speech_audio).decode('utf-8')
        socketio.emit('audio', {'audio': audio_data, 'session_id': session_id})
